define(function (require) {
    ...
    function getAllNearPlaces(latitude, longitude) {
        ...
        document.getElementById('searchATM').addEventListener('click', function () {
            ...
            fetch(proxyUrl + baseUrl + getAllPlaces + latitude + ',' + longitude + radiusAndTerm + placesKey, {'mode': 'no-cors', 'headers': 'origin'})
    ...
    function getSinglePlace(places) {
        ...
        for (var i = 0; i < places.length; i++) {
            ...
            fetch(proxyUrl + baseUrl + placeDetails + placeId + placesKey, {'mode': 'no-cors', 'headers': 'origin'})
                .then(response => response.json())
    ...
    function searchPlaceByUserInput(latitude, longitude) {
        ...
        document.getElementById('search-place-btn').addEventListener('click', function () {
            ...
            fetch(proxyUrl + baseUrl + textSearchUrl + location + '+' + placesKey, {'mode': 'no-cors', 'headers': 'origin'})
                .then(response => response.json())
